...
function getAllArticles(){//récupère tous les articles
    ...
    $query = "SELECT title, content, date, username, A.id
    FROM article A
    LEFT JOIN user U
    ON A.user_id = U.id
    ORDER BY date desc";
...
$query = "SELECT title, content, date, A.id
FROM article A
LEFT JOIN user U
ON A.user_id = U.id
...
function createArticle($username,$articleTitle,$articleContent){ //crée un article
    $mysql = connect();
